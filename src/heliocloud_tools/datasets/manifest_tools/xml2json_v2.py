...
def _title_from_top_description_short(dataset: ET.Element) -> Optional[str]:
    """
    Pull title from a <description short="..."> that is a *direct child* of <dataset>.
    (i.e., not nested under <access> or other blocks).
    """
...
def xml_to_catalog(xml_path: str) -> List[Dict[str, Any]]:
    ...
    for elem in root.iter():
        ...
        # NEW: prefer <description short="..."> direct child of dataset
        ...
        # Ensure "index" is the 3rd JSON key by constructing in-order
        obj: Dict[str, Any] = {
            "id": dataset_id,
            "title": title,
            "index": index,
